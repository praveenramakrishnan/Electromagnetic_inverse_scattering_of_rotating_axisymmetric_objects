...
the effects of motion. The values deduced with the first step where then
...
However, if the velocity of the rotating objects are small enough, then we may 
...
with the analytic or numerical solver for the rest case.
...
the candidate solutions until a termination criteria is satisfied.
...
To get an indication on how larger values of rotating speed can affect the 
...
The results with such an approach is shown in 
...
The geometry of the test case is shown Fig. \ref{fig:geometry_torus}.
...
of $10^{-4}$ and with geometric multigrid preconditioner.
